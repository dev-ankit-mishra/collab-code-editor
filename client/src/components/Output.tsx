...
export default function Output({onClick,children,bgClass,isLoading}:OutputProps):JSX.Element{
  return(
    <main className="w-[34rem] h-full p-4 flex flex-col gap-4">
      <Button className="h-8 px-3 w-20" onClick={onClick} disabled={isLoading}>{isLoading ? (<div className='w-5 h-5 rounded-full border-2 border-white border-t-transparent animate-spin'/>) : <><Play size={16}/> Run</>}</Button>
